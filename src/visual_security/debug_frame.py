...
Prints raw detections + bboxes, then runs PersonPPEChecker and shows
...
def run(image_path: str, detector_name: str, conf: float | None, cont_thr: float, iou_thr: float):
    ...
    h, w = img.shape[:2]
    result = det.analyze(img)   # ndarray diretto - niente disco

    if result.error:
        print(f"[ERROR] detector failed: {result.error}")
        return

    print(f"\n{result.model_name} - {len(result.detections)} detections  ({result.inference_time_ms:.1f} ms)")
...
def main():
    ...
    p.add_argument("--conf",        type=float, default=None, help="Detector confidence (default: backend default)")
    ...
    args = p.parse_args()
    run(args.image, args.detector, args.conf, args.containment, args.iou)

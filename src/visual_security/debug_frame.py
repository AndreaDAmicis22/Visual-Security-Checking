...
def run(image_path: str, yolo_model: str, conf: float, cont_thr: float, iou_thr: float):
    ...
    result = yolo.analyze(image_path)
    ...
    persons = checker.check(result.detections)
    ...
    if img is not None:
        for pr in persons:
            if pr.person_bbox:
                ...
                cv.putText(img, label, (x1, max(y1 - 6, 14)), cv.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                cy = y1 + 18
                for cat, req in PersonPPEChecker.FULL_PPE.items():
                    ...
                    cv.putText(
                        img, f"{'v' if ok else 'x'}{cat}({found}/{req})", (x1 + 4, cy), cv.FONT_HERSHEY_SIMPLEX, 0.38, c, 1
                    )
                    cy += 15
        for det in result.detections:
            if det.label.lower() != "person":
                ...
                if box:
                    ...
                    cv.putText(
                        img, det.label, (int(box[0]), int(box[1]) - 4), cv.FONT_HERSHEY_SIMPLEX, 0.35, (200, 200, 50), 1
                    )
...
def main():
    ...
    p.add_argument("--containment", type=float, default=0.30, help="Containment threshold for PPE→Person association")

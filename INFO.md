...
Inferenza zero-shot in-process: rileva oggetti **singoli** — Person, Helmet, Vest, Glasses, ogni singolo Glove, ogni singola Shoe, più gli item vietati (Cigarette) — da prompt testuali, con bbox pixel e confidence. Il testo matchato dal modello viene riportato alla categoria canonica tramite keyword (`_match_label`). Due filtri sull'output: **soglie di confidence per-classe** (`DETECTION_CONF`: Glasses 0.45, Cigarette 0.50, le altre alla soglia base) per tagliare i falsi positivi sulle classi difficili, e **NMS per-classe** (`nms_per_class`, IoU 0.5) per rimuovere i box duplicati sovrapposti.
...
- **Valutazione detector completa** (Grounding DINO vs OmDet-Turbo, mAP + statistiche temporali): cartella **`evaluation/`**, report in `evaluation/REPORT.md`. Usa il dataset **SH17** (annotazioni complete, occhiali inclusi), subset bilanciato di 259 immagini. Sintesi: GD mAP@.5 0.516 vs OmDet 0.482; GD più accurato, OmDet ~8.6× più veloce (unico near-real-time su CPU).
